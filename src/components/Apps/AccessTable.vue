    ...
    >
      ...
      <template slot-scope="{ row }" slot="access">
        <Button
          :class="[
            'access-table-button',
            row.collaborator.read || isDisabled('pull', row.repository)
              ? 'disabled'
              : '',
          ]"
          :disabled="
        ...
        <Button
          :class="[
            'access-table-button',
            row.collaborator.write || isDisabled('push', row.repository)
              ? 'disabled'
              : '',
          ]"
          :disabled="
    ...
    return {
      ...
      tableData: this.$store.state.cachedRepositories.map((repository) => {
        return {
          ...
          collaborator: {
            read: repository.collaborators.find(
              (collaborator) =>
                collaborator.login === this.$store.state.user.login
            )?.permissions?.pull,
            write: repository.collaborators.find(
              (collaborator) =>
                collaborator.login === this.$store.state.user.login
            )?.permissions?.push,
          },
